...
float observation_model(vector<float> landmark_positions, 
  ...
  for (auto observation : observations){
      if (pseudo_ranges.size() == 0){
          distance_prob *= Helpers::normpdf(observation,distance_max,observation_stdev);
      }
      else{
          auto pseudo_range = pseudo_ranges[0];
          distance_prob *= Helpers::normpdf(observation,pseudo_range,observation_stdev);
